...
public class GeoWars2D extends PApplet {
...
public void setup() {
  ...
  // Main Menu
  Play_Game    = new Button(width/5, height/5,    200, 40, "Play Game");
  Options      = new Button(width/5, height/5+50, 200, 40, "Options");
  
  // Options Menu
  Back_To_Menu = new Button(width/5, height/5,    200, 40, "Main Menu");
}
...
class Snake_Enemy {
  ...
  public float x_max = width;
  public float x_min = 0;
  public float y_max = height;
  ...
  public void check_collion(Snake_Link link) {
    rect(x_min, y_min, x_max-x_min, y_max-y_min);
    for (int i = 0; i < bullets.size(); i++) {
      Bullet b = (Bullet)bullets.get(i);
      if (b.type == false && b.x < x_max && b.x > x_min && b.y < y_max && b.y > y_min) {
      ...
      if (health <= 0 && this.active == true) {
        for (int j = 0; j < links.size(); j+=(size/2-5)) {
          Snake_Link link_e = (Snake_Link)links.get(j);
          explosion_add((int)link_e.x, (int)link_e.y, this.c);
        }
...
public float triangleArea(float p1, float p2, float p3, float p4, float p5, float p6) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--hide-stop", "GeoWars2D" };
    if (passedArgs != null) {
